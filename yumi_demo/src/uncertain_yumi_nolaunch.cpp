...
const std::string WRITE_TARGET_DIR = "/home/monica/Desktop/LOG_INFO/yumi_demo_data/visual_with_uncertainty";
const std::string MOTION_RESOURCES_DIR = "/home/monica/Desktop/LOG_INFO/motion_record";
const std::string MOTION_NAME = "motion10";
...
const std::string UNCERTAIN_FILE = "/home/monica/Desktop/LOG_INFO/motion_record/yumi_uncertain.txt";
...
/// @brief for uncertain
std::vector<double> limit_l;
std::vector<double> limit_u;
const bool withUncertainty = true;
const bool WRITE_REAL = true;   // record the position of point p and point q
...
void cal_distance_function_uncertain(std::vector<double> &start_state, std::vector<double> &vel,
                                     std::vector<double> &limit_l, std::vector<double> &limit_u);
...
void sample_uncertain(const moveit_msgs::MoveGroupActionGoal& msg);
void ReadMotionFile(std::string filename);
void ReadUncertainFile(std::string filename);
...
bool fclcallback(cInterval &T)
{
    ...
    fcl::ContinuousCollisionRequest req(num_max_iterations, 0.0001, fcl::GST_LIBCCD);
...
void ReadUncertainFile(std::string filename)
{
    ...
    infile.getline(buffer, 256);
//        std::cout << buffer << "\n";
    std::string s1(buffer, buffer+strlen(buffer));
    char cstr1[256];
    std::strcpy(cstr1, s1.c_str());
    char *p1 = std::strtok(cstr1, " ");
    while (p1!=0) {
        limit_l.push_back(atof(p1));
        p1 = strtok(NULL, " ");
    }

    infile.getline(buffer, 256);
//        std::cout << buffer << "\n";
    std::string s2(buffer, buffer+strlen(buffer));
    char cstr2[256];
    std::strcpy(cstr2, s2.c_str());
    char *p2 = std::strtok(cstr2, " ");
    while (p2!=0) {
        limit_u.push_back(atof(p2));
        p2 = strtok(NULL, " ");
    }
...
void init_()
{
    ...
    filename = UNCERTAIN_FILE;
    ReadUncertainFile(filename);
...
//    q << 0,0,0,1;
    ...
    if (WRITE_REAL) {
        ...
        q_rr = hm1 * q_rr;
        // p
        out << p_ll(0) << " " << p_ll(1) << " " << p_ll(2) << "\n";
        // q
        out << q_rr(0) << " " << q_rr(1) << " " << q_rr(2) << "\n\n";
        out.close();
    }
...
void cal_distance_function_uncertain(std::vector<double> &start_state, std::vector<double> &vel, std::vector<double> &limit_l, std::vector<double> &limit_u)
{
    std::vector<motion> subm;
    std::vector<result> res;
    double segment_interval = interval / num_parts;
...
//            ROS_INFO("joint %d: [%lf, %lf], v = %lf m/s", j+1, tmp.start_state[j], tmp.goal_state[j], tmp.velocity[j]);
        ...
        setupdualHM_movable_uncertain(7, T, tmp, limit_l, limit_u);
...
//    }
    ...
    cal_distance_function_uncertain(start_state.joint_state.position, vel, limit_l, limit_u);
    cur_Action++;
}
...
//    ros::Publisher dis_pub = n.advertise<std_msgs::Float64>("/plot/dist", 1);
//    ros::Publisher TMdis_poly_pub = n.advertise<std_msgs::Float64>("/plot/TM_poly", 1);
//    ros::Publisher TMdis_upper_pub = n.advertise<std_msgs::Float64>("/plot/TM_UB", 1);
//    ros::Publisher TMdis_lower_pub = n.advertise<std_msgs::Float64>("/plot/TM_LB", 1);
//    ros::Publisher diff_pub = n.advertise<std_msgs::Float64>("/plot/diff", 1);

    ros::Publisher data_all_pub = n.advertise <std_msgs::Float64MultiArray> ("data_all", 1);
...
//                ROS_INFO("TM distance bound: [%lf, %lf]", TMlower, TMupper);
                ...
                std::vector<double> array = {dis.data, TMpoly.data, TMupper.data, TMlower.data, diff.data};
                std_msgs::Float64MultiArray msg;
                msg.data = array;

                data_all_pub.publish(msg);
