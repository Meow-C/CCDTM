...
int cur_Motion = 0;
double cur_time = 0;
std::vector<std::vector<motion_TM2> > sub_motion;
...
std::string MOTION_NAME = "motion2";
...
//            ROS_INFO("joint %d: [%lf, %lf], v = %lf m/s", j+1, tmp.start_state[j], tmp.goal_state[j], tmp.velocity[j]);
        ...
        setupDualArm_TM2_HM_movable(7, T, tmp);

//        cTM2Vector4 p(0,0,0,1);
//        cTM2Vector4 q(0,0,0,1);
        cTM2Vector4 p(-0.0004065, 0.001969, 0.0259665,1);
        cTM2Vector4 q(-0.0004065, 0.001969, 0.0259665,1);
        p = M44multV14(TM2_HMleftfinger[0] ,p);
        q = M44multV14(TM2_HMrightfinger[1] ,q);
...
bool collisioncallback(cInterval &T, result &collisionresult) {
    ...
    std::string outfile = WRITE_TARGET_DIR + "/" + MOTION_NAME + "/" + "time_and_avg_TM2.txt";
    ...
    tra_num = 0;
    clock_t start = clock();
    fcl::FCL_REAL dis = BVHTraversal_TM2(0, 0, finger, finger);
    clock_t end = clock();
...
//        ROS_INFO("collide!\n");
    ...
    tra_num = 0;
    start = clock();
    dis = BVHTraversal_TM2(0, 0, finger, finger);
    end = clock();
...
int main(int argc, char **argv)
{
    ...
    std::string outfile = WRITE_TARGET_DIR + "/" + MOTION_NAME + "/" + ss.str() + "_TM2.txt";
