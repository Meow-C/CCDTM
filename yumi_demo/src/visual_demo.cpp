...
std::string MOTION_NAME = "motion6";
...
/// @brief reset the global parameters
void reset()
{
    /// global parameters in this file
    flag = false;
    vel.clear();
    cur_time = 0;
    ...
    for (int i = 0; i < 2; i++) {
        ...
        hm_hand[i].Zero();
        hm_leftfinger[i].Zero();
        hm_rightfinger[i].Zero();
    }

    sub_motion.clear();

    /// global parameters in fcl/include/tm/ModelParameters.h
//    init_frame_hm();
}
